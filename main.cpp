...
int main()
{
	...
	while (window.isOpen())
	{
		...
		// Отрисовка камеры
		int x = Fang.m_x + 8;		// Плюс половина ширины спрайта
		int y = Fang.m_y + 8;		// Плюс половина высоты спрайта
		if (Fang.m_x < (WINDOW_WIDTH / 2) - 8)	// Чтобы камера не уходила за левый край экрана
			x = WINDOW_WIDTH / 2;	// Разница с условием, равна половине ширины спрайта( иначе будут резкие прыжки камеры)
		if (Fang.m_y > ((MAP_HEIGHT * 16) / 2) + 28)	// Одталкиваемся от высоты карты а не экрана
